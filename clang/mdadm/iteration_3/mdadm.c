...
int mdadm_read(uint32_t start_addr, uint32_t read_len, uint8_t *read_buf)  {
  ...
  while (bytes_read < read_len) { // `read_len` must be at least 1
    ...
    //   if   the requested JBOD blck is in the cache then read it from the cache
    //   else read the JBOD blck from JBOD and insert it into the cache
    if (cache_lookup(disk, blck, buf) == 1)
      memcpy(read_buf, &buf[ofst], chunk);                                           // read chunk of `buf` into `read_buf`
    else {
      assert(jbod_operation(op(IGND, IGND, disk, JBOD_SEEK_TO_DISK),  NULL) == 0);   // seek to correct disk of `start_addr`
      assert(jbod_operation(op(IGND, blck, IGND, JBOD_SEEK_TO_BLOCK), NULL) == 0);   // seek to correct blck of `start_addr`
      assert(jbod_operation(op(IGND, IGND, IGND, JBOD_READ_BLOCK),     buf) == 0);   // read the entire blck into `buf`
      if (cache_enabled())
        assert(cache_insert(disk, blck, buf)                                == 1);   // insert JBOD blck into cache
      memcpy(read_buf, &buf[ofst], chunk);                                           // read chunk of `buf` into `read_buf`
    }
...
int mdadm_write (uint32_t start_addr, uint32_t write_len, const uint8_t *write_buf) {
  ...
  while (bytes_written < write_len) { // `write_len` must be at least 1
    ...
    uint8_t tmp[256];
    if (cache_lookup(disk, blck, tmp) == 1)
        cache_update(disk, blck, buf);
    else
      if (cache_enabled())
        assert(cache_insert(disk, blck, buf) == 1);
